...
namespace GXPEngine {
    ...
    public class Gizmos {
        ...
        /// </summary>
        public static void DrawLine(float x1, float y1, float x2 = 0, float y2 = 0, GameObject space = null,
            uint color = 0, byte width = 0) {
            if (Game.main == null) {
        ...
        /// </summary>
        public static void DrawPlus(float x, float y, float radius, GameObject space = null, uint color = 0,
            byte width = 0) {
        ...
        /// </summary>
        public static void DrawCross(float x, float y, float radius, GameObject space = null, uint color = 0,
            byte width = 0) {
        ...
        /// </summary>
        public static void DrawRay(float x, float y, float dx, float dy, GameObject space = null, uint color = 0,
            byte width = 0) {
        ...
        /// using DrawLine.
        /// </summary>
        public static void DrawRayAngle(float x, float y, float angleDegrees, float length, GameObject space = null,
            uint color = 0, byte width = 0) {
        ...
        public static void DrawArrow(float x, float y, float dx, float dy, float relativeArrowSize = 0.25f,
            ...
            DrawLine(x, y, x + dx, y + dy, space, color, width);
            DrawLine(x + dx, y + dy, x + dx * (1 - relativeArrowSize) - dy * relativeArrowSize,
                y + dy * (1 - relativeArrowSize) + dx * relativeArrowSize, space, color, width);
            DrawLine(x + dx, y + dy, x + dx * (1 - relativeArrowSize) + dy * relativeArrowSize,
                y + dy * (1 - relativeArrowSize) - dx * relativeArrowSize, space, color, width);
        }
        ...
        /// The relativeArrowSize is the size of the arrow head compared to the arrow length.
        /// </summary>
        public static void DrawArrowAngle(float x, float y, float angleDegrees, float length,
            float relativeArrowSize = 0.25f, GameObject space = null, uint color = 0, byte width = 0) {
            float dx = Mathf.Cos(angleDegrees * Mathf.PI / 180) * length;
            float dy = Mathf.Sin(angleDegrees * Mathf.PI / 180) * length;
        ...
        public static void DrawRectangle(float xCenter, float yCenter, float width, float height,
            GameObject space = null, uint color = 0, byte lineWidth = 0) {
            DrawLine(xCenter - width / 2, yCenter - height / 2, xCenter + width / 2, yCenter - height / 2, space, color,
                lineWidth);
            DrawLine(xCenter - width / 2, yCenter + height / 2, xCenter + width / 2, yCenter + height / 2, space, color,
                lineWidth);
            DrawLine(xCenter - width / 2, yCenter - height / 2, xCenter - width / 2, yCenter + height / 2, space, color,
        ...
        /// used. These can be set using SetStyle, SetColor and SetWidth.
        /// </summary>
        public static void RenderLine(float x1, float y1, float x2, float y2, uint pColor = 0xffffffff,
