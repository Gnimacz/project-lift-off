...
namespace TiledMapParser {
    ...
    public class TiledLoader {
        ...
        public static EasyDraw CreateTextField(TiledObject obj, bool addCollider = true, bool highQualityText = true) {
            ...
            EasyDraw message = new EasyDraw((int) Mathf.Ceiling(obj.Width * scaleMultiplier),
                (int) Mathf.Ceiling(obj.Height * scaleMultiplier), addCollider);
            // TODO: Cache fonts?
            ...
            message.TextFont(new Font(obj.textField.font, Mathf.Round(obj.textField.fontSize * scaleMultiplier), f,
                GraphicsUnit.Pixel));
            message.graphics.TextRenderingHint = TextRenderingHint.AntiAliasGridFit; //AntiAlias;

            // Set text alignment:
            message.TextAlign(
                obj.textField.horizontalAlign == "left"
                    ? CenterMode.Min
                    : (obj.textField.horizontalAlign == "right" ? CenterMode.Max : CenterMode.Center),
                obj.textField.verticalAlign == "top"
                    ? CenterMode.Min
                    : (obj.textField.verticalAlign == "bottom" ? CenterMode.Max : CenterMode.Center)
            );

            // Set color:
            uint col = obj.textField.Color;
            message.Fill(
                Color.FromArgb((int) (col & 255), (int) ((col >> 24) & 255), (int) ((col >> 16) & 255),
        ...
        public static void DrawText(EasyDraw textCanvas, string text) {
            ...
            // Set Text alignment:
            float textX =
                textCanvas.HorizontalTextAlign == CenterMode.Min
                    ? 0
                    : (textCanvas.HorizontalTextAlign == CenterMode.Max ? pixelWidth : pixelWidth / 2f);
            float textY =
                textCanvas.VerticalTextAlign == CenterMode.Min
                    ? 0
        ...
        public static void ChangeOrigin(Sprite spr, float newOriginRelativeX = 0.5f, float newOriginRelativeY = 0.5f,
            float oldOriginX = 0, float oldOriginY = 0) {
            ...
            Vector2 newCenter = spr.TransformPoint((newOriginRelativeX - oldOriginX) * imageWidth,
                (newOriginRelativeY - oldOriginY) * imageHeight);
        ...
        public static void SetPositionRotationScaleOrigin(Sprite newSprite, TiledObject obj,
            float normalizedOriginX = 0.5f, float normalizedOriginY = 0.5f) {
            newSprite.scale = 1;
            ...
            foreach (TiledObject obj in group.Objects) {
                ...
                if (_manualObjects.Contains(obj.Type)) {
                    ...
                    if (tileSet == null || tileSet.Image == null)
                        throw new Exception(
                            "The Tiled map contains unembedded tilesets (.tsx files) - please embed them in the map");
                    ...
                    if (autoInstance && obj.Type != null) {
                        try {
                            ...
                            anim = (AnimationSprite) _callingAssembly.CreateInstance(obj.Type, false,
                                BindingFlags.Default, null,
                                new object[] {
                                    Path.Combine(_foldername, tileSet.Image.FileName), tileSet.Columns, tileSet.Rows,
                                    obj
                                }, null, null);
                            if (anim == null)
                                throw new Exception("Class with name " + obj.Type +
                                                    " not found in the calling assembly. Check namespace?");
                        }
                        catch (Exception error) {
                            Console.WriteLine(
                                "Couldn't automatically create an AnimationSprite object from the Tiled (image) object with ID {0} and type {1}.\n Error: {2}",
                                obj.ID, obj.Type, error.Message);
                    ...
                    if (anim == null) {
                        anim = new AnimationSprite(Path.Combine(_foldername, tileSet.Image.FileName), tileSet.Columns,
                            tileSet.Rows, -1, false, addColliders);
                    }
                    ...
                    if (autoInstance && obj.Type != "") {
                        try {
                            ...
                            //	new object[] { obj });
                            // Necessary when using separate assemblies (engine=DLL):
                            newSprite = (Sprite) _callingAssembly.CreateInstance(obj.Type, false, BindingFlags.Default,
                                null,
                            ...
                            Console.WriteLine(
                                "Couldn't automatically create a sprite object from the Tiled (shape) object with ID {0} and type {1}.\n Error: {2}",
                                obj.ID, obj.Type, error.Message);
                            // Check the console for more information!
                            throw error;
                        }
